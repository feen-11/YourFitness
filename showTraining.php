...
<head>
  ...
  <title><?= h($training['created'])?> <?= h($training['trainingName'])?> - YourFitness</title>
  ...
  <div class="user-show">
      <div class="container">
        <div class="user-info container">
          <h2 class="text-center heading"><?= h($training['created'])?>：<?= h($training['trainingName'])?>の詳細</h2>
          <div class="container user-info-show">
            <p class=""><i class="fas fa-utensils"></i>トレーニング名：<?= h($training['trainingName']) ?></p>
            <p><i class="fas fa-fire fa-lg"></i> 消費カロリー：<?= h($training['burnCalorie']) ?>kcal</p>
            <p><i class="fab fa-pagelines fa-lg"></i> 日時：<?= h($training['created']) ?></p>
            <div class="justify-content-center col-sm-12 col-md-6 mx-auto text-center">
              <a class="btn btn-primary edit-link" href="editTraining.php?id=<?= $training['trainingId']?>">編集</a>
